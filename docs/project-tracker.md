...
- `M0-021` Reproducible dependency/tooling gate.
  ...
  - existing main-branch CI now resolves and installs the approved dependency set successfully;
  - CI generated the real `pnpm-lock.yaml` artifact from Node 24 + pnpm 11.18.0;
  - existing Security workflow passed its dependency audit;
  - CI then exposed a formatting-policy failure before lint/typecheck/tests;
  - formatting scope is being corrected to executable source/tests/package manifests rather than long-form planning Markdown;
  - task remains open until the generated lockfile is committed verbatim, existing CI/security installs are switched to `--frozen-lockfile`, and the quality run is verified.

### NEXT — exact execution order

1. Finish `M0-021`: commit the CI-generated `pnpm-lock.yaml` verbatim, freeze installs in the two existing workflows, and verify formatting → lint → typecheck → unit → build → Playwright.
...
- `M0-021` The lockfile exists as a verified CI-generated artifact; source-control transfer must preserve it verbatim. Do not recreate or hand-edit it merely to close the task.
...
| Toolchain drift breaks reproducibility | High | Exact pins + committed lockfile + frozen installs after M0-021 |
...
| Dependency security audit | ✅ Existing Security workflow passed after dependency fix |
| Formatting | ⚠️ CI exposed scope/style issue; correction is in progress |
| Lint | ⬜ Waiting behind formatting gate |
| Typecheck | ⬜ Waiting behind formatting gate |
| Unit tests | ⬜ Waiting behind formatting gate |
| Build | ⬜ Waiting behind formatting gate |
| Playwright harness | ⬜ Waiting behind formatting gate |
...
> `M0-021 — inspect the main-branch CI after the formatting-scope correction; fix the next real quality-gate failure if one appears, while preserving the CI-generated pnpm-lock.yaml verbatim for source-control commit. Complete frozen installs before starting M0-030.`
